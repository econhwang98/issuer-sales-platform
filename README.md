...
- **EBITDA 한계 고지**: OpenDART `fnlttSinglAcntAll`은 현금흐름표의 소계만 주고 감가상각비 같은
  조정 세부항목은 대부분 담지 않습니다. 그런 기업은 상각비를 더할 수 없어 EBITDA가 영업이익과
  같아지며, 검토보고서에 그 사실과 영향(EBITDA 관련 지표가 보수적으로 계산됨)을 명시합니다.
...
| 변수 | 기본값 | 뜻 |
| --- | --- | --- |
| `FINANCIAL_SHARD_LIMIT` | 300 | 한 번에 재무제표를 받을 기업 수 (기업당 3콜) |
