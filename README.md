...
이벤트 점수는 사건의 크기가 아니라 **선제 영업 기회로서의 가치**입니다.

유상증자·전환사채·회사채 발행 공시는 그 시점에 이미 주관사와 투자자가 정해져 있습니다.
딜이 확정된 뒤에 나오는 공시이므로 선제 접촉 대상이 아니라 이미 놓친 건입니다.
따라서 후속 조달 가능성이라는 잔여 가치만 남기고 낮게 둡니다(유상증자 45, 회사채 38).

반대로 대규모 투자·인수 결정, 신용등급 하향은 **앞으로 자금이 필요해질** 신호이고
아직 주관사가 정해지지 않았을 가능성이 커서 높게 둡니다(신용등급 하향 92, 대규모 투자 90).
채무불이행·자본잠식·회생절차는 즉시 자본확충 수요이므로 100입니다.

## v13 고도화 핵심

- **상장 구분**: 코스피·코스닥은 상장, 코넥스·기타법인은 비상장으로 나눕니다.
  판별 근거는 OpenDART 회사개요의 `corp_cls`이며, 코넥스도 `stock_code`를 가지므로
  `stock_code` 유무로는 추정하지 않습니다.
- **비상장사 포함**: 최근 공시 이력이 있는 비상장 법인을 유니버스에 더합니다(`UNLISTED_MAX`).
- **mock 점수 제거**: 재무제표를 받은 기업은 `scoring_engine`의 8개 항목
  (부채비율·유동비율·당좌비율·차입금의존도·이자보상배율·영업현금흐름·OCF/ICF 조합·현금 순감소)
  으로 룰 점수를 냅니다. 아직 받지 못한 기업은 업종 위험도와 공시 이벤트만 쓰는 임시 점수이며,
  화면과 보고서에 **대체지표**로 표시됩니다. 어느 쪽인지 필터로도 구분됩니다.
- **재무제표 단계적 수집**: 우선순위 상위부터 `FINANCIAL_SHARD_LIMIT`만큼 받아
  `financials/{corp_code}.json`으로 저장하고, 나머지는 다음 실행에서 이어 받습니다.
  `fnlttSinglAcntAll`이 1회 호출로 3개년을 주므로 기업당 3콜이면 5개년 + 최신분기가 채워집니다.
- **재무지표는 빈칸으로 두지 않습니다**: 구하지 못한 지표는 추정값으로 채우는 대신 표에서 뺍니다.
  상각비를 확인할 수 없으면 EBITDA와 그 파생 지표(EBITDA마진, 순차입금/EBITDA, EBITDA/금융비용)를
  제외하고 **매출액·EBIT(영업이익)·당기순이익**까지만 싣습니다. 값이 하나도 없는 지표 행과
  기간 열도 표에서 제거합니다. 연도별로 연결/별도 제출이 갈려 손익계산서가 빈 구간은
  다른 재무제표 구분(CFS/OFS)으로 한 번 더 조회해 채웁니다.
- **기업 검토보고서**: 상세 패널의 버튼을 누른 기업만 그 자리에서 생성해 A4 인쇄용으로 띄웁니다.
  생성한 보고서는 브라우저 IndexedDB에 누적하고, 재요청 시 지문(정책버전·재무 기준·점수·리스크·
  등급·이벤트 목록)을 비교해 **변경이 없으면 기존 보고서를 그대로** 제공합니다.
  달라졌을 때만 새 판을 만들고 무엇이 바뀌었는지 표시합니다.
- **일간 변화**: 직전 스냅샷 대비 신규 진입·점수 급등·상승·하락을 필터와 배지로 봅니다.
- **접촉 관리**: 접촉 상태·메모·관심(★)을 브라우저에 저장하고 필터와 CSV로 내보냅니다.
- **로딩**: `daily_snapshot.json`을 `no-store`에서 `no-cache`로 바꿔 재방문 시 ETag 304로 끝납니다.
  스냅샷 자체도 절반으로 줄였습니다(16.4MB → 7.5MB, gzip 전송 기준 0.5MB).
  화면이 쓰지 않는 `source_status`, 3사 모두 무등급일 때의 등급 배열, 요약 구간의 정형문
  `rationale`, 빈 값 필드를 빼고 indent 없이 씁니다. 뺀 값은 모두 화면 쪽에 되만드는 경로가 있습니다.

## 실행 제어 (GitHub Actions Variables)

| 변수 | 기본값 | 뜻 |
| --- | --- | --- |
| `FINANCIAL_SHARD_LIMIT` | 900 | 한 번에 **새로** 받을 재무제표 수 (기업당 3콜). 캐시 재사용은 한도를 쓰지 않아 실행마다 커버리지가 넓어짐 |
| `FINANCIAL_TIME_BUDGET_MIN` | 35 | 재무 신규 수집에 쓸 최대 시간(분). 초과하면 받은 만큼만 쓰고 다음 실행이 이어받음 |
...
재무 파서와 점수 교체 단계는 API 키 없이 합성 응답으로 검증됩니다.

```bash
python tests/test_financial_engine.py
python tests/test_financial_stage.py
```
...
- 자동 산출값은 영업 검토용 선별 신호입니다. 실행 전 DART 원문, 뉴스 원문, 신용등급 원천자료, 담당자 검토가 필요합니다. 전화번호는 OpenDART 회사개요 기준이므로 실제 IR 담당 부서 번호와 다를 수 있습니다.
